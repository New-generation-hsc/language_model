...
def load_data(data_dir, max_word_length, eos='+'):
        ...
        with codecs.open(os.path.join(data_dir, fname + '.txt'), 'r', 'utf-8') as f:
            for line in f:
                ...
                for word in line.split():
                    ...
                    word_tokens[fname].append(word_vocab.feed(word)) # append the index of current word in the vocabulary

                    char_array = [char_vocab.feed(c) for c in '{' + word + '}']
                    char_tokens[fname].append(char_array) # append the characters array  in current word into the char vocabulary
    ...
    print('number of tokens in train:', len(word_tokens['train'])) # the word token is the corpus size with duplicated words
    ...
    for fname in ('train', 'valid', 'test'):
        ...
        char_tensors[fname] = np.zeros([len(char_tokens[fname]), actual_max_word_length], dtype=np.int32) # initialize the vector to 0

        """
        It deal with such situation
        words = [4, 5, 6, 7, 8]
        char = [11, 12, 13, 14, 0] just like padding 0
        """
...
class DataReader:
    ...
    def __init__(self, word_tensor, char_tensor, batch_size, num_unroll_steps):
        ...
        print("============verifying the next word=================")
        print("source data:", word_tensor[:20])
        print("target data:", ydata[:20])
        print("====================================================")
...
if __name__ == "__main__":
    ...
    count = 0
    for source, target in DataReader(wt['test'], ct['test'], 20, 35).iter():
        print("source:")
        print(source)
        print("target:")
        print(target)
